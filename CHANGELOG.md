...
- **Vaults: rename, extract, drag-out, and streaming for large files.** Inside an unlocked vault you can now **rename** files and whole sub-folders — instant, because it only re-keys the encrypted index (no file is re-encrypted, so renaming a folder holding gigabytes is immediate) — and get a file back out two ways: **extract** a decrypted copy to a folder of your choice, or **drag it straight out** to Explorer or the desktop (a native OS drag; the original stays in the vault either way, and an existing file at the destination is never overwritten). Encryption now also **streams**: adding or opening a multi-gigabyte file processes it one 1 MiB chunk at a time instead of loading the whole thing into memory. A new on-disk blob format carries this — each chunk is sealed independently, with a per-chunk index and a final-flag inside the authenticated data, so re-ordering or truncation is caught rather than silently returning scrambled or short bytes. **Vaults created by v0.1.21 (whole-file blobs) are detected and still open unchanged.** See CLAUDE.md §43.
...
- **Vault sub-folders.** The encrypted index is keyed by each file's `/`-separated path relative to the vault root, so encrypting a folder tree encrypts every file at every depth while the blobs stay flat on disk (the folder structure never touches the disk). `vault_encrypt_existing` recurses; `vault_list(sub_path)` reconstructs one level; `vault_add_file(dest_dir)` / `vault_delete_folder(sub_path)` manage folders; `VaultView` gains a breadcrumb + folder navigation. See CLAUDE.md §43.
- 9 new unit tests (`like_escape`, `resolve_temp`, 4 covering the sharing path validation, plus `norm_sub`, `immediate_children`, and `collect_plaintext_files` for the vault folder tree). Local Rust toolchain aligned to the CI's `stable` (clippy 1.97) so pipeline lints are caught before push; the `for_kv_map` clippy error in `ocr.rs` is fixed.
...
- **Quick filter `/` in the file list** — press `/` in any folder to open a small floating bar that filters the visible entries by substring (case-insensitive, file name only). Live `matched / total` counter, `↑/↓` cycle the selection within the filtered set without losing input focus, `Enter` opens the arrow-navigated entry or the first match (folder = navigate, file = open), `Esc` closes (also via outside Esc when the input is blurred). Filter clears automatically on folder navigation, workspace switch, and viewer transitions. Per-pane state, so dual-pane gives each pane its own independent filter. The new bar carries `data-is-header` to stay out of the rubber-band selection's pointer capture, and the `/` interception sits **before** the existing letter-key type-ahead branch so it doesn't fall through as a name-jump for files starting with `/`. Empty-state precedence is now `quickFilter > search > tagFilter > emptyFolder` — the most recently-typed reason for "nothing here" wins, with a "Clear filter" action surfaced directly in the empty state. Applies in both list and grid layouts.
- **Quick-start popover anchored under the app logo** — click the logo (top-left of the titlebar) to open a 300 px panel with three sections: current workspace header (icon + name), the 5 most-recently-touched paths from the activity log, and a workspaces switcher. Mouse-driven counterpart to the Ctrl+K command palette. Closes via outside-click (`mousedown` ignored when the target is inside the popover or the logo button itself, so the toggle on the logo doesn't fight the close handler), Esc (registered in capture phase + `stopImmediatePropagation` so dismissing the popover does NOT also clear the file selection underneath), or any item-navigation. The workspace switcher replicates `Sidebar.tsx`'s `handleSwitch` flow exactly (`update_context` to persist the leaving workspace's open tabs + last path, `set_active_context`, store-side `switchWorkspace`, then `list_directory` of the restored `last_path`) so no state is lost across switches. Single intentional role: "where do I want to go right now?" — no settings link, no about, no changelog, by design (those live in Settings and the command palette).

### Improved

- **Taskbar icon fills the canvas more aggressively** — `public/nxs-icon.svg` viewBox tightened from `48 96 416 416` to `60 112 376 376`. The artwork now occupies ~89% of the rasterized canvas (vs ~81% before), matching the visual weight of neighbor icons (VS Code, Files App, Opera) in the Win11 taskbar without clipping the Fluent ambient shadow tail. Symmetric ~5.3% horizontal padding, ~9-10% vertical. All 42 OS icon raster files (Windows `.ico`, macOS `.icns`, the four main PNGs, 9 Square* Store tiles, 16 iOS variants, 15 Android mipmaps) regenerated via `npx tauri icon`. **Important nuance**: the in-app `<NxsLogo>` React component's viewBox is deliberately kept at the previous `48 96 416 416` — at 22 px in the titlebar sitting next to the workspace switcher / window chrome, a tighter crop reads as cramped. The two surfaces are now intentionally desynced; commented in both source files so a future contributor doesn't re-sync them by reflex.
- **Recents lists (Ctrl+K palette and the new quick-start popover) filter out noise** — paths buried inside `.git/objects/…`, `node_modules/…`, `target/…`, `__pycache__/…`, `.venv/…`, `AppData/…`, `$Recycle.Bin/…`, `.vs/.idea/…`, `.next/.nuxt/.turbo/…` no longer crowd out the useful entries. The activity log records every `navigated` event regardless of where the user ended up (usually because an IDE / build tool / sync agent reached into the tree) and those entries were dominating the top 10 slots. Filename patterns also filtered: Office lock files (`~$file.docx`), Word temp files (`~WRL0001.tmp`, `~WRA*`, `~WRD*`), LibreOffice / OpenOffice locks (`.~lock.file.odt#`), browser-download partials (`*.crdownload/.part/.partial`), generic editor temps and backups (`*.tmp/.temp/.bak/.orig/.swp/.swo` and trailing-tilde `file~`), OS metadata (`Thumbs.db/desktop.ini/.DS_Store`), and hash-suffixed sync intermediates (extension is 8+ hex chars, e.g. `Memo.docx.a8f2b3c4d5` from Dropbox / OneDrive / atomic-save flows). Conservative — `package-lock.json`, `Cargo.lock`, `bar.git/` (a literal folder, not `.git`), `IMG_20240101_a8b2c4d5.jpg` (hash inside the name, not the extension) are NOT filtered. The full filter list lives in `src/utils/noisePaths.ts` with rationale and false-positive examples; add a new entry only when the pattern shows up repeatedly for multiple users.

### Fixed

- **Automation rules with numeric conditions (`size_gt`, `size_lt`, `age_gt_days`) couldn't be saved** — `create_automation_rule` returned `invalid args 'rule' for command 'create_automation_rule': missing field 'value'` from the Rust side. Cause: the TypeScript `Condition` discriminated union serialized these variants with `bytes` / `days` field names, while the Rust `Condition` enum uniformly expects `value` for every variant. Aligned the TS type union, `newCondition`, both preset rules (`Archive old screenshots`, `Clean tmp`), and `ConditionRow`'s read/write logic to send `{ kind, value }`. The bug only got reported visibly for `age_gt_days` (the user-triggering case) but `size_gt`/`size_lt` were equally broken — same root cause. No DB migration needed because no rule with the wrong shape could ever have been saved successfully in the first place. CLAUDE.md §39 also updated to drop the misleading `{bytes}` / `{days}` shorthand that implicitly seeded the wrong field names.

### Internal

- **New `src/utils/noisePaths.ts`** — single source of truth for the "what's noise in a recents list?" predicate. Used by both `CommandPalette.tsx` and `QuickStartPopover.tsx`. Two-tier matching: (a) strict path-segment match against a small known-noise list (`.git`, `node_modules`, etc.) — case-insensitive, segment-strict so `bar.git` ≠ `.git`; (b) filename patterns (prefix / suffix / equality / hash-extension regex). Adding a new entry is one line in the appropriate const array.
- **New `src/components/QuickStartPopover.tsx`** — self-contained, 7 new EN/FR i18n strings (`quickStartTitle`, `quickStartWorkspace`, `quickStartNoWorkspace`, `quickStartRecent`, `quickStartNoRecent`, `quickStartWorkspaces`, `quickStartActive`), animations reuse the existing `modal-content-in` keyframes. Workspace switch logic kept local rather than promoted to a store helper for the moment — if a third call site appears, extract.
- **`Titlebar.tsx`** — logo wrapped in a `<button data-tauri-drag-region="false">` to break out of the parent's window-drag region for this specific surface; the rest of the titlebar still drags. Local `popoverOpen` + `anchorRect` state, no store entry.
- **`FileList.tsx`** — new `prefilteredEntries` memo split off the existing `visibleEntries` pipeline so the quick-filter X/Y counter denominator reflects post-hidden/post-tag-filtering counts rather than the unfiltered `paneEntries`. The quick filter is applied AFTER tag/hidden filters and BEFORE sort, so sort always runs on the narrowed set (cheap on big folders). Global keydown effect now also depends on `quickFilter` (added to deps array).
- **5 new i18n strings for the quick filter** (`quickFilterPlaceholder`, `quickFilterCountTitle`, `quickFilterNoMatches`, `quickFilterNoMatchesHint`, `quickFilterClear`) added to both EN and FR maps alongside the 7 quick-start ones.
- **`.gitignore`** — added explicit entries for `nxs-updater.key` and `nxs-updater.key.pub` so the private signing key has a permanent safety net against accidental staging. The on-disk files are working copies; the canonical secret lives in the GitHub `TAURI_SIGNING_PRIVATE_KEY` repo secret.
